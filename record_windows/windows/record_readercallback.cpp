				...
				if (SUCCEEDED(hr))
				{
					...
					if (SUCCEEDED(hr))
					{
						...
						if (SUCCEEDED(hr))
						{
							...
							// of `this`, mirroring UpdateState's pattern.
							// `this->m_recordEventHandler` is nulled inside
							// Recorder::Dispose() while the lambda is still queued, so
							// re-reading it through `this` crashes with a null deref at
							// EventStreamHandler::Success.
